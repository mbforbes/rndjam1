...
WeightUpdateFn = Callable[
    [FloatTensor, FloatTensor, float, Union[float, FloatTensor], float, float], float
]
...
# util functions (consider pulling out)
...
def ridge_gradient(
    w: FloatTensor, x: FloatTensor, y: FloatTensor, lmb: float
) -> FloatTensor:
    """
    See the README section for the derivation:
...
def gradient_descent(
    ...
    for epoch in range(epochs):
        ...
        if epoch % report_interval == 0:
            ...
            record[epoch] = loss
            print(
                " .. epoch {}, lr: {:.4f}, loss: {:.4f} (gradient mag: {:.4f}) (0 ws: {})".format(
...
def coordinate_descent(
    ...
    y_int: IntTensor,
    lmb: float,
    weight_update_fn: WeightUpdateFn,
    loss_fn: LossFn,
    settings: CDSettings,
) -> Tuple[FloatTensor, Record]:
    """
    ...
    for epoch in range(epochs):
        ...
        for j in range(d):
            ...
            w_diff = w_j_old - w[j]
            r += (
                x[:, j : j + 1].matmul(w_diff.view(-1, 1).t())
                if multiclass
                else w_diff * x[:, j]
            )
    return w, record
...
def accuracy(
    x: FloatTensor, y: IntTensor, w: FloatTensor, eval_fn: EvalFn
) -> Tuple[int, int, float]:
    """
    Arguments:
        x: 2d (N x D) input data
...
def scalar():
    ...
    w, record = gradient_descent(
        train_x, train_y, -1, ols_loss, ols_gradient, ols_gd_settings
    )
    ...
    w, record = coordinate_descent(
        ...
        train_y,
        dummy,
        ols_cd_weight_update,
        ols_loss,
        {"epochs": 150, "report_interval": 10},
    )
    report(
        "[scalar] Coordinate descent (train)",
        w,
        train_x,
        train_y,
        dummy,
    ...
    for lmb in [0.2]:
        ...
        report(
            "[scalar] Ridge analytic (train) lambda={}".format(lmb),
            w,
            train_x,
            train_y,
            lmb,
        ...
        report(
            "[scalar] Ridge analytic (val) lambda={}".format(lmb),
            w,
            val_x,
            val_y,
            lmb,
    ...
    for lmb in [0.2]:
        w, record = gradient_descent(
            train_x, train_y, lmb, ridge_loss, ridge_gradient, ridge_gd_settings
        )
        report(
            "[scalar] Ridge GD (train) lambda={}".format(lmb),
            w,
            train_x,
            train_y,
            lmb,
        ...
        report(
            "[scalar] Ridge GD (val) lambda={}".format(lmb),
            w,
            val_x,
            val_y,
            lmb,
    ...
    for lmb in [0.2]:
        w, record = coordinate_descent(
            train_x, train_y, lmb, ridge_cd_weight_update, ridge_loss, ridge_cd_settings
        )
        report(
            "[scalar] Ridge CD (train) lambda={}".format(lmb),
            w,
            train_x,
            train_y,
            lmb,
        ...
        report(
            "[scalar] Ridge CD (val) lambda={}".format(lmb),
            w,
            val_x,
            val_y,
            lmb,
    ...
    for lmb in [0.2]:
        w, record = gradient_descent(
            train_x, train_y, lmb, lasso_loss, lasso_gradient, lasso_gd_settings
        )
        report(
            "[scalar] Lasso GD (train) lambda={}".format(lmb),
            w,
            train_x,
            train_y,
            lmb,
        ...
        report(
            "[scalar] Lasso GD (val) lambda={}".format(lmb),
            w,
            val_x,
            val_y,
            lmb,
    ...
    for lmb in [0.2]:
        ...
        report(
            "[scalar] Lasso CD (train) lambda={}".format(lmb),
            w,
            train_x,
            train_y,
            lmb,
        ...
        report(
            "[scalar] Lasso CD (val) lambda={}".format(lmb),
            w,
            val_x,
            val_y,
            lmb,
